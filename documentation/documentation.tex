...
Music and emotion are strongly related to each other \cite{Krumhansl2002} but emotion is never considered in existing recommendation systems. In our approach we implemented a system that recommends music by emotion. The emotion of the song is determined using a classifier that is trained on audio-features extracted from the song. The extracted emotion is then used to recommend a song to the user.
